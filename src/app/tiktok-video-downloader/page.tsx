...
import { DownloaderWorkspace } from "@/components/downloader/downloader-workspace";
...
export async function generateMetadata(): Promise<Metadata> {
  return buildToolPageMetadata(await requireToolPage("tiktok-video-downloader"));
}

export default async function TikTokVideoDownloaderPage() {
  const page = await requireToolPage("tiktok-video-downloader");
  const ui = page.ui as {
    safetyControls: {
      description: string;
      icon: keyof typeof safetyIcons;
      title: string;
    }[];
      ...
      <section className="border-y border-ink/10 bg-cream py-16 sm:py-20">
        <div className="container">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-sm leading-6 text-ink/55">{ui.safetyDescription}</p>
          </div>

          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {ui.safetyControls.map((control) => {
              const ControlIcon = safetyIcons[control.icon];
              return (
                <article
                  key={control.title}
                ...
                >
                  <span className="grid size-11 place-items-center rounded-2xl bg-acid">
                    <ControlIcon className="size-5" aria-hidden="true" />
                  </span>
